...
;(() => {
    ...
    MOREBUTTON.addEventListener('click', function () {
        console.log(this)
        let start = showDataCount
        let end = showDataCount + 8
        for (let i = start; i < end; i++) {
            showDataCount++
            console.log(result[i])
            renderPosition(result[i])
        }
    })
    getAjax(requestURL)
        .then((res) => {
    ...
    function renderPosition(data) {
        let name = document.createTextNode(`${data.stitle}`)
        let li = document.createElement('li')
        let img = document.createElement('img')
        let p = document.createElement('p')
        img.src = data.file.split('jpg')[0] + 'jpg'
        img.alt = "is img"
        p.appendChild(name)
        li.appendChild(img)
        li.appendChild(p)
